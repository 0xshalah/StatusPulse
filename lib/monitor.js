...
export async function pingWithRetry(ep) {
  // DNS pre-check via public resolver to avoid EdgeOne-local DNS cache
  let dnsFailed = false
  try {
  ...
  } catch { /* DNS check unavailable, proceed with ping */ }
...
export async function getDashboard(db) {
  ...
  // Fetch only the 30 most-recent pings per endpoint (server-side $slice) instead of
  // transferring the full ping history and trimming in Node — keeps the payload bounded
  // and lets the { endpointId, timestamp } index drive the sort.
  const grouped = await db.collection('pings').aggregate([
    { $match: { endpointId: { $in: epIds } } },
    { $sort: { timestamp: -1 } },
    { $group: { _id: '$endpointId', pings: { $push: '$$ROOT' } } },
    { $project: { pings: { $slice: ['$pings', 30] } } },
  ]).toArray()
  const pingsByEp = {}
  for (const g of grouped) pingsByEp[g._id] = g.pings.map(clean)
  const data = []
