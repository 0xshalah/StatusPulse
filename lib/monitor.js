...
export async function pingWithRetry(ep) {
  ...
  let last = { statusCode: 0, responseTime: 0, errored: true, attempts: 0 }
  for (let i = 0; i < 3; i++) {
    ...
    try {
      ...
      if (r.status === ep.expectedStatus) return last
    } catch (e) {
...
export async function getSettings(db) {
  ...
  return { slackWebhookUrl: s.slackWebhookUrl || '', notifyOnDown: s.notifyOnDown !== false, notifyOnDegraded: s.notifyOnDegraded !== false, notifyOnRecovery: s.notifyOnRecovery !== false }
}
...
async function sendSlackNotification(db, ep, verdict, prevVerdict) {
  const settings = await getSettings(db)
  if (!settings.slackWebhookUrl) return null
...
export async function recordPing(db, ep, result) {
  ...
  if (verdict !== prevVerdict) {
    sendSlackNotification(db, ep, verdict, prevVerdict).catch(() => {})
  }
  return ping
}
...
export async function testUrl(url, expectedStatus = 200) {
  ...
  const verdict = failed ? (result.errored ? 'down' : 'degraded') : result.responseTime > 2000 ? 'degraded' : 'up'
...
export async function getDashboard(db) {
  ...
  for (const ep of eps) {
    const recent = (await db.collection('pings').find({ endpointId: ep.id }).sort({ timestamp: -1 }).limit(30).toArray())
      .map(clean)
      .reverse()
...
export async function getStatus(db) {
  ...
  for (const ep of eps) {
    const rollups = (await db.collection('rollups').find({ endpointId: ep.id }).sort({ date: 1 }).toArray()).map(clean)
    const recent = (await db.collection('pings').find({ endpointId: ep.id, timestamp: { $gte: new Date(Date.now() - 48 * 3600 * 1000) } }).sort({ timestamp: 1 }).toArray()).map(clean)
    const latest = recent.length ? recent[recent.length - 1] : null
    const verdict = displayVerdict(ep, latest)
    if (verdict === 'up' || verdict === 'degraded') healthy++
    // build 30-day heatmap from rollups
    const map = {}
