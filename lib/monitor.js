...
export async function pingWithRetry(ep) {
  ...
  for (let i = 0; i < CONFIG.ping.maxAttempts; i++) {
    ...
    try {
      ...
      clearTimeout(t)
      // Check content match if expectedContent is configured
      let contentMismatch = false
      if (ep.expectedContent && r.status === ep.expectedStatus) {
        try {
          const text = await r.clone().text()
          contentMismatch = !text.includes(ep.expectedContent)
        } catch { /* can't read body, skip check */ }
      }
      last = { statusCode: r.status, responseTime: Date.now() - start, errored: false, attempts: i + 1, contentMismatch }
