      ...
      <DialogContent className="sm:max-w-[500px]">
            ...
            return (
              <div key={s.id} className="flex flex-1 items-center">
                <div className="flex flex-col items-center gap-1">
                  <div className={`flex h-9 w-9 items-center justify-center rounded-full border transition-colors ${done ? 'border-primary bg-primary text-primary-foreground' : active ? 'border-primary text-primary' : 'border-border text-muted-foreground'}`}>
                    {done ? <Check className="h-4 w-4" /> : <Icon className="h-4 w-4" />}
                  </div>
                  <span className={`text-[10px] uppercase tracking-wide ${active ? 'text-foreground' : 'text-muted-foreground'}`}>{s.title}</span>
                </div>
                {i < STEPS.length - 1 && <div className={`mx-2 h-px flex-1 ${step > s.id ? 'bg-primary' : 'bg-border'}`} />}
              </div>
            )
          })}
        </div>
        ...
        <div className="min-h-[210px]">
          <AnimatePresence mode="wait">
            {step === 1 && (
              <Slide key="s1">
                <div className="space-y-2"><Label htmlFor="name">Display name</Label><Input id="name" placeholder="e.g. Payments API" value={form.name} onChange={(e) => set('name', e.target.value)} /></div>
              ...
              <Slide key="s2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {INTERVALS.map((i) => (
                      <button key={i.v} type="button" onClick={() => set('interval', i.v)} className={`rounded-md border px-3 py-1.5 font-mono text-xs transition-colors ${Number(form.interval) === i.v ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:text-foreground'}`}>{i.l}</button>
                    ))}
                  </div>
            ...
            {step === 4 && (
              <Slide key="s4">
                <div className="rounded-xl border border-border bg-muted/30 p-5 text-center">
                  {testState.status === 'loading' && <div className="flex flex-col items-center gap-2 py-4 text-muted-foreground"><Loader2 className="h-6 w-6 animate-spin text-primary" /><p className="text-sm">Pinging {form.url}…</p></div>}
                  {testState.status === 'done' && testState.result && (() => {
                    const v = statusOf(testState.result.verdict)
                    return (
                      <div className="flex flex-col items-center gap-3 py-2">
                        <div className="flex h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: `${v.color}22`, color: v.color }}><Activity className="h-6 w-6" /></div>
                        <p className="font-display text-lg font-semibold" style={{ color: v.color }}>{v.label}</p>
                        <div className="flex gap-6 font-mono text-sm">
                          <span>status <b>{testState.result.statusCode || 'ERR'}</b></span>
                          <span>time <b>{fmtMs(testState.result.responseTime)}</b></span>
                        </div>
                        <Button variant="outline" size="sm" onClick={runTest} className="mt-1 gap-1.5"><RefreshCw className="h-3.5 w-3.5" /> Re-test</Button>
                      </div>
                    )
                  })()}
                </div>
...
function Slide({ children }) {
  return <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-4">{children}</motion.div>
}
